"""
YATCA Auto-Reply Extension.
Automatically sends the final response to the Telegram user when
the agent's process chain ends. Includes retry logic on failure.
"""
...
class YatcaAutoReply(Extension):
    ...
    async def execute(self, loop_data: LoopData = LoopData(), **kwargs):
        ...
        try:
            ...
            PrintStyle.error(f"YATCA auto-reply error: {format_error(e)}")
        finally:
    ...
    async def _send_reply(
        ...
        if failures <= MAX_SEND_RETRIES:
            ...
            PrintStyle.error(
                f"YATCA send failed {failures} times, giving up: {error}"
            )
            context.log.log(
                type="error",
                heading="YATCA send failed (max retries reached)",
                content=error,
            )
...
def _notify_agent_of_failure(
    ...
    msg = context.agent0.read_prompt(
        "fw.yatca.send_failed.md",
        error=error,
        attempt=str(attempt),
        max_retries=str(MAX_SEND_RETRIES),
    )
    context.log.log(type="error", heading="YATCA send failed", content=error)
