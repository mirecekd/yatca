"""
YATCA Bot Lifecycle Manager.
Starts, restarts, or stops bots whenever plugin settings change.
Runs on each job_loop tick.
"""
...
class YatcaBotManager(Extension):
    ...
    async def execute(self, **kwargs: Any) -> None:
        ...
        # Avoid installing aiogram on idle ticks when YATCA is not configured.
        ...
        for bot_cfg in bots_cfg:
            ...
            try:
                ...
                _on_id = partial(_make_handler(handle_id), bot_name=name, bot_cfg=bot_cfg)
                ...
                _on_nudge = partial(_make_handler(handle_nudge), bot_name=name, bot_cfg=bot_cfg)
                _on_context = partial(_make_handler(handle_context), bot_name=name, bot_cfg=bot_cfg)
                _on_tasks = partial(_make_handler(handle_tasks), bot_name=name, bot_cfg=bot_cfg)
                ...
                instance = create_bot(
                    ...
                    on_command_help=_on_help,
                    on_command_clear=_on_clear,
                    on_command_status=_on_status,
                    on_command_id=_on_id,
                    on_command_stop=_on_stop,
                    on_command_resume=_on_resume,
                    on_command_nudge=_on_nudge,
                    on_command_context=_on_context,
                    on_command_tasks=_on_tasks,
                    on_command_project=_on_project,
                ...
                if mode == "webhook":
                    ...
                    if webhook_url:
                        ...
                        PrintStyle.error(
                            f"YATCA ({name}): webhook mode requires webhook_url"
                        )
                        continue
                else:
                    await start_polling(instance)

                PrintStyle.success(f"YATCA ({name}): bot started in {mode} mode")

            except Exception as e:
                PrintStyle.error(
                    f"YATCA ({name}): failed to start: {format_error(e)}"
                )
