"""
YATCA Response Intercept Extension.
Intercepts the 'response' tool to:
- Capture attachments and keyboard for later delivery
- Send inline progress updates when break_loop=false
"""
...
class YatcaResponseIntercept(Extension):
    ...
    async def _send_inline(self, context, tool, response: Response):
        ...
        if error:
            result = agent.read_prompt("fw.yatca.update_error.md", error=error)
        else:
            result = agent.read_prompt("fw.yatca.update_ok.md")
